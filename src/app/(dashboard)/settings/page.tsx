    ...
    const SettingToggle = ({
        ...
        <div className="flex items-start justify-between gap-4 border-b border-gray-100 py-4">
            ...
            <button
                ...
                className={`w-12 h-6 flex items-center rounded-full p-1 transition ${settings[id] ? "bg-lamaSky" : "bg-gray-200"
                    }`}
                aria-pressed={settings[id]}
            >
                <span
                    className={`w-4 h-4 bg-white rounded-full shadow transform transition ${settings[id] ? "translate-x-6" : "translate-x-0"
                        }`}
                />
            </button>
        </div>
